...
def classify_core(bucket: str, term: str) -> str:
    ...
    if bucket == "理论":
        ...
        if "独立自主" in t or "和平共处五项原则" in t:
            return "独立自主和平外交与和平共处五项原则"
        if "正确义利观" in t:
            return "坚持正确义利观"
    if bucket == "经济全球化":
...
def analysis_flow(bucket: str, core: str) -> list[str]:
    ...
    if bucket == "理论":
        return [
            ...
            "该写哪个术语：凡是解释国际关系背后原因的，优先找国家利益、共同利益、综合国力、国际竞争实质、独立自主等解释性原理；“共同利益是合作的基础”可解释合作为什么成立，含新型国际关系、国际秩序、全球治理方向的合作共赢转入政治多极化，含贸易、市场、产业链供应链、资源流动、开放平台的合作共赢转入经济全球化，含中国担当、共享机遇、发展中国家民生的合作共赢转入中国桶。",
