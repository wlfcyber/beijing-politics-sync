...
- `ClaudeCode-first thick content rule`: for full-book宝典 work, ClaudeCode B is expected to produce the richer content mine before final fusion. Codex A still runs its own evidence/content lane, but Codex must not use a thinner Codex candidate as the default final body when ClaudeCode has source-backed fuller entries. The successful philosophy宝典 pattern was ClaudeCode supplying much of the content density and Codex supplying source裁决, framework normalization, student/audit separation, external-review digestion, Governor/Confucius gates, and Word validation.
...
8. Run the ClaudeCode thick-content gate from `references/claudecode-first-fusion-workflow.md`: verify that ClaudeCode did not merely summarize or review, that every evidence/control row is body/index/blocked, that every user framework node has real question挂载 or a blocker, and that the content is richer than a thin Codex candidate before fusion proceeds.
9. After every completed phase or substantial milestone, run the Phase Boundary GPT Commander Gate from `references/whole-book-sop.md`: write a sanitized phase report, send it to GPT-5.5 Pro when available, save the raw reply, digest accepted/rejected advice locally, and continue from Codex-owned tasks. If the web/other thread stalls, log the fallback and keep moving under the local protocol instead of freezing the book run.
10. After every fixed content-review trigger (`outline`, `section_batch`, `final_markdown`, `word_pdf`), run the GPT-5.5 Pro Content Review Gate from `references/whole-book-sop.md`. Send the generated content itself in chunks when needed, collect concrete content corrections, verify each substantive correction locally, patch the artifact, and repeat while `must_fix_content` or transfer-blocking `should_fix_transfer` issues remain. Missing triggers require fallback or user waiver.
11. Send the evidence-locked fusion pack to Claude Opus 4.7 Adaptive for teaching-text成品化: structure, readability, transfer logic, answer sentence fluency, and student-facing wording. Codex must reject or return for evidence checks any new unsupported content.
12. Fuse into a student artifact organized by the user's framework, not by suite order, unless the user explicitly asks otherwise. For 选必三思维, this means `思维类型 -> 三性/三新/子方法 -> 全部对应模拟题`; for 推理, this means `推理题型树 -> 全部对应题`; do not fall back to a paper-order流水账 body.
13. Generate final Markdown and Word through the simplified document pipeline in `references/document-pipeline.md`. If final output is `.docx`, also use the `documents` skill, render/export for visual QA when the renderer is available, and use Microsoft Word validation when available.
14. Run Governor checks, GPT content review, then Confucius checks. Any FAIL or unresolved high-risk question sends the work back to the relevant lane.
15. Finish only when fresh verification evidence exists and `FINAL_ACCEPTANCE_REPORT.md` states the final paths, validation evidence, known exclusions, and remaining user questions if any.
...
- Do not let a thin Codex candidate become the final body while a fuller source-backed ClaudeCode body exists. The philosophy宝典 success pattern requires ClaudeCode thick-content mining first, then Codex evidence-verified fusion and cleanup.
- Do not organize final student宝典正文 by paper/question order when the user's framework calls for a framework-first document. Question-order ledgers and traceability matrices belong in audit, not as the main student structure.
...
- `references/claudecode-first-fusion-workflow.md`: postmortem-derived workflow from the successful 必修四哲学宝典 run; locks the ClaudeCode thick-content-mine-first plus Codex fusion/audit/Word-validation pattern and the framework-first final structure.
