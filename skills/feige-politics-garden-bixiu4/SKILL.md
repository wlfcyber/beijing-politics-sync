---
name: feige-politics-garden-bixiu4
description: "Use when Codex needs 飞哥政治庄园-必修四 for Beijing Gaokao politics 必修四 philosophy/culture research: district paper inventory, answer keys, scoring rubrics/marking rules, lecture reports, material-to-knowledge trigger chains, choice-question wrong-option patterns, cumulative Markdown/Word deliverables, long-running multi-role workflows, strict governor review, cache-first validation, and Windows/Mac/GitHub syncing."
---
...
Apply these guardrails before adding or keeping any material-to-knowledge chain:

- Do not use "等角度" as a catch-all to add a new principle. If a rubric lists A/B/C "等角度", principle D can enter the trigger field only when D or a standard synonym appears in the rubric, marking report, or scoring source. Otherwise mention it only as a weak reasoning note.
- Distinguish evidence levels exactly: formal rubric/scoring rules, marking report/evaluation PPT, teacher-version reference answer, and lecture PPT are not interchangeable. Teacher reference answers and lecture PPTs must be labeled as non-rubric support.
- For these high-risk terms, require explicit source support before marking them as trigger knowledge: 辩证否定, 量变质变, 主次矛盾, 矛盾主次方面, 两点论重点论, 主流支流, 价值观导向.
- Choice-question correct-option chains must be labeled separately from main-question scoring chains. Use "correct option corresponds to..." instead of "stable scoring trigger" for choice questions.
- Every new source-grounded row must be traceable to grep-able corpus text or a recorded evidence path. If not, mark NEED_EVIDENCE rather than PASS.
- After adding a batch, compare top-level framework sections that say "暂无稳定挂点" or "待补" against newly added rows and update contradictory headings.
- "拓展答案" and "参考答案" are not scoring rubrics. Do not write "评分细则要求" unless the source is actually a scoring rubric, marking report, or user-confirmed scoring source.
- Governor PASS cannot replace row-level evidence. A worker or governor summary is a pointer to evidence, not the evidence itself.
...
```powershell
$env:PYTHONUTF8='1'
python "$env:USERPROFILE\.codex\skills\feige-politics-garden-bixiu4\scripts\preprocess_corpus_cache.py" --render-scan-pages
```
...
& 'C:\Users\Administrator\AppData\Local\Python\pythoncore-3.14-64\python.exe' `
  'C:\Users\Administrator\.codex\skills\feige-politics-garden-bixiu4\scripts\prepare_analyst_workspace.py'
```
...
```powershell
$env:PYTHONUTF8='1'
python "$env:USERPROFILE\.codex\skills\feige-politics-garden-bixiu4\scripts\init_book_research_workspace.py" --book "<book-name>" --project-name "飞哥的政治庄园"
```
...
```powershell
$env:PYTHONUTF8='1'
python "$env:USERPROFILE\.codex\skills\feige-politics-garden-bixiu4\scripts\sync_artifacts.py" export --workspace "C:\Users\Administrator\Desktop" --skill-root "$env:USERPROFILE\.codex\skills\feige-politics-garden-bixiu4"
```
