...
def style_document(document: Document):
    ...
    if "Hyperlink" in styles:
        styles["Hyperlink"].font.color.rgb = RGBColor(30, 30, 30)
        styles["Hyperlink"].font.underline = False

    for style_name in ["TOC 1", "TOC 2", "TOC 3", "toc 1", "toc 2", "toc 3"]:
        if style_name in styles:
            styles[style_name].font.name = FONT_BODY
            styles[style_name]._element.rPr.rFonts.set(qn("w:eastAsia"), FONT_BODY)
            styles[style_name].font.size = Pt(10.5)
            styles[style_name].font.color.rgb = RGBColor(30, 30, 30)
            styles[style_name].font.underline = False
