# Web GPT-5.5 Pro Prompt - Batch01 Trial3

请在 ChatGPT 网页端新建一个独立对话，确认当前模式为 `GPT-5.5 Pro` 后使用本 prompt。

项目：选必二《法律与生活》框架实验。

禁止继承其他项目上下文；禁止把其他正在运行的项目、对话、线程、文件结论带入本任务。

你是真实的 GPT-5.5 Pro，本轮任务不是写最终答案，而是参加飞哥政治庄园选必二《法律与生活》框架制定委员会。你不是证据权威；你的建议必须等待 Codex 回到本地题面和评分细则裁决。
...
4. 证据边界：reference answer 不能升格为 formal scoring source。
...
## 0. Mode Confirmation
请写明：本报告基于用户可见的 GPT-5.5 Pro 网页端新对话生成。
...
请重点压力测试：这个候选框架是否过早、是否漏掉纯程序题/效力题/比较题/选择题、是否会把法律模块误做成法考，或误做成必修三宏观法治。
