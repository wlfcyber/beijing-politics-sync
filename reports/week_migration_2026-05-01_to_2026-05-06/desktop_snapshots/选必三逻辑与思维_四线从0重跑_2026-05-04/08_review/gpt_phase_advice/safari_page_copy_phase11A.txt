# Phase11A GPT-5.5 Pro Content Review Packet

## Run Context

- Project: 选必三《逻辑与思维》思维部分 + 推理部分，从0重跑到最终教学文档。
- Current allowed lanes: Codex local evidence/review + GPT-5.5 Pro web review only.
- User constraint: Claude desktop/app, ClaudeCode CLI, Claude Opus, and ClaudeCode Lane B are temporarily suspended because the memberships dropped. Do not require or count those gates in this Phase11A review.
- Current object: a review-only 29-row student body, not final, not Word/PDF, not expanded from the remaining evidence rows.
- Hard prohibition: do not authorize Word/PDF/final PASS/终稿/最终稿/宝典成品. Do not auto-expand same-type index rows or 74-row evidence pool into正文.

## What I Need From GPT-5.5 Pro

Please perform a concrete content review of the actual student-facing body below. Do not praise generally; pressure-test it. Return exactly one verdict at the top:

1. `MUST_FIX_PHASE11A_CONTENT_NO_EXPANSION` if any of the 29 rows has a concrete concept/content/answer-logic error that must be fixed before moving on.
2. `GO_PHASE11B_CONTROLLED_EXPANSION_NO_WORD_NO_FINAL` if the 29-row body is content-safe enough to begin controlled expansion/source repair.
3. `HOLD_NO_EXPANSION_NEEDS_SOURCE_REPAIR` if the current body cannot be judged without specific source repairs.

For each issue, use this format:

- QID or visible title:
- Severity: must_fix_content / should_fix_transfer / optional_style / source_repair_needed
- Problem:
- Concrete correction or local verification needed:
- Whether this blocks Phase11B controlled expansion:

## Review Priorities

Check especially:

- Q-2025顺义一模-7: true error must remain 大项不当扩大; 小项不当扩大 appears only as wrong option wording.
- Q-2025丰台期末-7: must remain a boundary trap, not a 选必三超前思维正例.
- Q-2026顺义一模-19-2: primary line must be 科学思维三特征, not typical 三段论.
- Q-2024朝阳二模-19-1/19-2: should use 第一空/第二空 language; no audit/source/file wording.
- Q-2024朝阳一模-20-1/20-2 and Q-2026通州期末-19-2: sufficient/necessary conditional rules must stay separated.
- Q-2026丰台一模-18-2: preserve 甲 necessary conditional valid; 乙 syllogism 大项不当扩大 invalid.
- Q-2025海淀二模-20: keep angle-pool treatment, not fixed mandatory three points.
- Hard-excluded rows must not become正文: Q-2024西城一模-11, Q-2025海淀二模-12, Q-2025海淀二模-13, Q-2026顺义一模-3.
- Student-facing cleanliness: no audit/provenance/model terms, no source paths, no “评标/参考答案/可从...角度作答/pass/filled/correct_option_chain”.
- Transfer quality: each entry should help a weak student move from material signal to method/rule to answer action.
...
## QID Hard-Lock Recheck

```md
# Phase11 QID Lock Recheck

- PASS: 顺义一模 Q7 keeps 大项不当扩大 as true error and 小项不当扩大 only as A项错误表述.
- PASS: no forbidden Q11 `B=①④` pairing in student body.
- PASS: `Q-2024西城一模-11` raw QID absent from student body.
- PASS: `Q-2025海淀二模-12` raw QID absent from student body.
- PASS: `Q-2025海淀二模-13` raw QID absent from student body.
- PASS: `Q-2026顺义一模-3` raw QID absent from student body.

```

## Same-Type No-Expansion Check

```md
# Phase11 Same-Type Index No-Expansion Check

- control body rows: 29
- visible headings: 29
- same-type index accidental heading expansions: 0
- status: PASS
...
## Student Body For Content Review
...
- 答题动作：沿“杂多现象→抽出核心因果→回到整体规律”链条理解共变法的认识价值。
