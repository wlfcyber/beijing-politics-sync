...
RUN = ROOT / "V4_终审修订_2026-05-09"
AUDIT = RUN / "01_v4_audit"
...
V3_DOCX = ROOT / "V3_终审修订_2026-05-09" / "08_final" / "2026北京高考政治选必一《当代国际政治与经济》六要素原框架答题术语宝典_主观题版_V3.docx"
V3_MD = ROOT / "V3_终审修订_2026-05-09" / "08_final" / "final_v3.md"
...
FINAL_MD = FINAL / "final_v4.md"
FINAL_DOCX = FINAL / "2026北京高考政治选必一《当代国际政治与经济》六要素原框架答题术语宝典_主观题版_V4.docx"
...
PLACEHOLDER_PATTERNS = ["具体材料事实", "材料中的“ ”", "材料中的 直接对应", "直接对应", "应写出", "这一术语用来解释", "这一术语用来说明", "相邻术语不能直接解释", "作答时要", "政治环境层", "国家利益方面对应", "回答国家利益方面"]
TEMPLATE_PATTERNS = ["PDF 小框架", "本题优先触发", "本题优先写", "踩分词集中在", "因为踩分词集中在", "不优先写相邻术语，是因为", "考题增补术语", "考题增补说明", "旧稿", "后台", "补丁题链", "见对应题号", "答案见本题正确项"]
ANSWER_META_PATTERNS = ["题目追问", "材料呈现", "就要想到", "触发", "先归入", "再落到", "直接对应", "应写出", "本题优先", "踩分词", "说明该术语", "作为可作答表达", "政治环境层", "不是 PDF 原框架", "学生记住"]
NONPDF_WHY_REPLACEMENTS = {
    "治理赤字": "治理困境",
    "全球性挑战": "共同挑战",
    "逆全球化": "脱钩断链等逆向潮流",
    "零和博弈": "对抗思维",
...
def clean_fact(text, limit=52, keep_nonpdf=False):
    ...
    if not keep_nonpdf:
        for term, repl in NONPDF_WHY_REPLACEMENTS.items():
...
def answer_core_patterns(t: TermDef):
    return {
        "era_peace": ["和平与发展", "时代主题", "时代潮流"],
        "era_challenge": ["霸权主义", "强权政治"],
        "security": ["国家安全", "发展和安全", "经济安全", "科技安全", "数据安全"],
        "common_interest": ["共同利益", "共同需求"],
        "reasonable_concern": ["兼顾他国合理关切", "正确义利观", "互利共赢"],
        "global_m": ["民生", "就业", "收入", "基础设施"],
        "global_g": ["经济复苏", "经济增长", "发展动能"],
        "global_huan": ["要素", "流动", "投资", "消费", "市场", "产业"],
        "global_ying": ["营商环境", "市场化", "法治化", "国际化"],
        "global_pei": ["资源配置", "配置效率"],
        "global_trade": ["贸易投资", "自由化", "便利化", "贸易壁垒", "市场准入"],
        "global_direction": ["开放", "包容", "普惠", "平衡", "共赢"],
        "global_open": ["开放型经济", "高水平对外开放", "引进来", "走出去", "制度型开放"],
        "global_jian": ["两个市场", "两种资源", "国内国际"],
        "new_rel": ["新型国际关系", "相互尊重", "公平正义", "合作共赢"],
        "governance": ["全球治理", "共商共建共享", "治理体系"],
        "community": ["人类命运共同体", "命运共同体", "命运与共"],
        "unilateral": ["单边主义", "贸易保护主义", "霸权主义", "强权政治", "真正的多边主义", "反补贴税", "贸易壁垒", "多边主义"],
        "multi": ["多极化", "国际格局", "阵营对抗"],
        "democracy": ["国际关系民主化", "平等参与", "共同商量", "共同决定"],
        "policy": ["独立自主", "和平外交政策", "和平共处", "维护世界和平", "促进共同发展"],
        "wisdom": ["中国智慧", "中国方案", "中国理念"],
        "responsibility": ["大国责任", "建设性作用", "负责任大国", "发展中国家"],
        "history_line": ["综合国力", "国际地位", "影响力", "话语权"],
        "main_line": ["习近平外交思想", "根本遵循", "行动指南"],
        "un_charter": ["联合国宪章", "宪章宗旨", "宪章原则"],
        "un_place": ["联合国", "多边主义", "安理会"],
    }.get(t.key, [t.title])


def answer_has_core(t: TermDef, answer: str):
    return any(p in answer for p in answer_core_patterns(t))


def entry_has_core(t: TermDef, r):
    combined = "；".join(r.get(k, "") for k in ["材料触发点", "答案落点", "踩分词", "对应答题术语", "旧稿核心标题"])
    if t.key in {"unilateral", "democracy", "multi", "governance", "community", "new_rel"}:
        return any(p in combined for p in answer_core_patterns(t))
    return True


def answer_fact(r, limit=54):
    text = sanitize_sentence(r.get("V3材料触发点") or r.get("材料触发点", ""), keep_nonpdf=True)
    heuristics = [
        ("气候风险", "全球气候治理面临气候风险和责任分歧"),
        ("回应全球南方求平等", "发展中国家群体要求平等参与全球治理"),
        ("发展中国家群体求平等", "发展中国家群体要求平等参与全球治理"),
        ("沙伊和解", "中国通过劝和促谈、气候合作和人工智能治理倡议参与全球治理"),
        ("全球人工智能治理倡议", "中国通过劝和促谈、气候合作和人工智能治理倡议参与全球治理"),
        ("为构建周边命运共同体", "中国推动构建周边命运共同体并支持发展中国家联合自强"),
        ("共同迈向现代化", "中国推动构建周边命运共同体并支持发展中国家联合自强"),
        ("国际组织会议讲话", "中国通过联合国、金砖国家、二十国集团、博鳌亚洲论坛等平台参与全球治理"),
        ("双重标准", "中国反对双重标准并主张国际法平等适用"),
        ("主权平等", "全球治理倡议强调主权平等、国际法治和多边主义"),
        ("小而美", "小而美项目帮助共建国家改善发展条件"),
        ("不变侧列出反对霸权主义", "新中国外交一贯反对霸权主义和强权政治"),
        ("全球经济、安全、气候和治理挑战", "国际局势变乱交织、全球挑战增多"),
        ("完善全球气候治理首先要回答治理原则", "全球气候治理需要坚持平等、公平公正和多边协商"),
        ("发达国家与发展中国家责任分歧", "全球气候治理需要坚持平等、公平公正和多边协商"),
    ]
    for needle, repl in heuristics:
        if needle in text:
            return repl
    text = re.sub(r"^(材料写|材料显示|要求说明|设问中的|设问落在|答案明确|首先要回答|先说明)", "", text)
    text = re.sub(r"引出[^。；]{0,60}", "", text)
    text = re.sub(r"对应[^。；]{0,60}", "", text)
    text = re.sub(r"\s+", "", text).strip("，。；; ")
    if not text:
        text = "相关事实"
    return (text[:limit] + "…") if len(text) > limit else text
...
def term_specific_answer(t: TermDef, r):
    fact = answer_fact(r)
    if t.key == "era_peace":
        return f"{fact}顺应和平与发展的时代主题，回应各国共同发展愿望和世界发展大势。"
    if t.key == "era_challenge":
        return "材料中的单边霸凌、强权逻辑、恃强凌弱等体现霸权主义和强权政治仍然存在，是当今国际社会面临的挑战。"
    if t.key == "security":
        return f"{fact}关系国家安全，必须统筹发展和安全，维护经济安全、科技安全等重要领域安全。"
    if t.key == "common_interest":
        return f"{fact}说明国家间存在共同利益，共同利益是国家间合作的基础，各方能够在合作中实现共同发展。"
    if t.key == "reasonable_concern":
        return f"{fact}说明我国在维护本国利益的同时兼顾他国合理关切，坚持互利共赢的合作取向。"
    if t.key == "global_m":
    ...
    if t.key == "unilateral":
        return f"面对{fact}，我国反对单边主义和贸易保护主义，反对霸权主义和强权政治，主张坚持真正的多边主义。"
    if t.key == "multi":
        return f"{fact}体现世界多极化发展要求，应坚持平等有序的多极化，反对阵营对抗和霸权逻辑。"
    if t.key == "democracy":
        return f"{fact}体现国际事务应由各国平等参与、共同商量，推动国际关系民主化。"
    if t.key == "policy":
    ...
    if t.key == "main_line":
        return f"习近平外交思想为新时代中国特色大国外交提供根本遵循和行动指南，{fact}体现新时代中国特色大国外交的实践要求。"
    if t.key == "un_charter":
        return f"{fact}应遵循联合国宪章宗旨原则，在联合国框架下推进国际合作。"
    if t.key == "un_place":
        return f"联合国是践行多边主义的最佳场所，{fact}说明应依托联合国平台推进多边合作。"
    return clean_answer_text(r.get("答案落点", "")) or f"{fact}能够扣合“{t.title}”。"
...
def purify_answer(t: TermDef, r):
    ...
    if t.key == "unilateral" and "2025西城期末" in source:
        return "欧盟对中国电动汽车征收高额反补贴税，本质上是贸易保护主义，会扭曲国际市场竞争，不利于正常经贸往来。"
    if t.key == "reasonable_concern" and "2026朝阳期末" in source:
        return "中国特色大国外交既要维护我国主权、安全、发展利益和海外利益，也要在开放合作中兼顾各方合理关切，为推进中国式现代化营造稳定外部环境。"
    if t.key == "multi" and "2025延庆一模" in source:
        return "“脱钩断链”背离世界多极化发展趋势和国际合作要求，中国反对阵营对抗和霸权逻辑，主张在平等有序的多极化中维护全球产业链供应链稳定。"
    if t.key == "un_charter" and "2024东城二模" in source:
        return "中国坚持联合国宪章宗旨和原则，通过维和、安理会协商和国际合作以和平方式解决争端，有利于维护国际和平与安全。"
    if t.key == "un_place" and "2024东城二模" in source:
        return "联合国是践行多边主义的最佳场所，中国在联合国框架下参与维和、安理会协商和国际合作，有利于维护国际和平与安全。"
    if t.key == "era_challenge":
        return "材料中的单边霸凌、强权逻辑、恃强凌弱等体现霸权主义和强权政治仍然存在，是当今国际社会面临的挑战。"
    base = clean_answer_text(r.get("答案落点", ""))
    if base and answer_has_core(t, base):
        return base
    return term_specific_answer(t, r)
...
def why_text(t: TermDef, r):
    source = r.get("题源", "")
    if t.key == "security" and "2026朝阳期中" in source:
        return "材料一边写人工智能带来效率提升和发展动能，一边写数据泄露、模型缺陷、伦理失控等风险，题目又要求处理人工智能发展中的重要关系，所以归在理论中的国家安全，落到国家安全是最高国家利益。共同利益只能解释国际合作基础，不能替代国家安全的底线逻辑；本题得分关键是把发展收益、安全风险和维护经济安全、科技安全、数据安全连成一条链。"
    signal = clean_fact(r["V3材料触发点"], 44, keep_nonpdf=False) or "材料信号"
    relation = {
        "era_peace": "设问或材料强调时代大势、共同发展愿望和合作共赢，所以归在时代背景中的机遇，落到和平与发展是时代的主题；共同利益只能解释合作为什么能够成立，不能解释做法为什么符合时代大势。",
        "era_challenge": "材料强调单边霸凌、强权逻辑或贸易胁迫，所以归在时代背景中的挑战，落到霸权主义和强权政治；政治多极化中的单边、民主等术语是回应路径，不替代这里对挑战本身的识别。",
        "common_interest": "材料强调共同问题、共同需求或利益交汇，所以归在理论中的共同利益，落到共同利益是国家间合作的基础；时代主题说明合作顺应大势，但本题得分重心是合作为什么能够成立。",
        "reasonable_concern": "材料同时出现维护自身利益和照顾合作方合理诉求，所以归在理论中的共同利益，落到维护本国利益的同时兼顾他国合理关切；共同利益是合作基础，本术语进一步说明合作中怎样处理利益关系。",
        "global_m": "材料落在就业、收入、基础设施或民众获得感，所以归在经济全球化中的民；国侧重国家层面的增长，民侧重普通民众生活改善。",
        "global_g": "材料落在共建国家经济复苏、增长和发展动能，所以归在经济全球化中的国；民侧重居民生活，本题得分重心是国家经济发展。",
        "global_huan": "材料强调人员、商品、资金、技术、服务流动以及投资消费、市场开拓和产业升级，所以归在经济全球化中的环；配只写资源配置效率，不能替代经济环节的流动和升级。",
        "global_ying": "材料强调市场化、法治化、国际化或投资经营便利，所以归在经济全球化中的营；贸资侧重贸易投资制度便利，营侧重经营环境。",
        "global_pei": "材料强调资源在更大范围内配置和效率提升，所以归在经济全球化中的配；环侧重经济要素流动和产业环节，不替代资源配置效率。",
        "global_trade": "材料强调贸易投资、壁垒、市场准入或自由便利，所以归在经济全球化中的贸资；开侧重我国开放水平，贸资侧重贸易投资障碍如何降低。",
        "global_direction": "材料强调开放、包容、普惠、平衡、共赢和共同繁荣，所以归在经济全球化中的总；开侧重我国开放举措，总侧重经济全球化的发展方向。",
        "global_open": "材料强调高水平对外开放、开放型经济、规则标准对接或走出去，所以归在经济全球化中的开；兼侧重两个市场两种资源，本题得分重心是开放水平和开放型经济。",
        "global_jian": "材料强调国内国际两个市场、两种资源和内外联动，所以归在经济全球化中的兼；开侧重开放政策，兼侧重资源和市场统筹。",
        "new_rel": "材料强调相互尊重、公平正义、合作共赢的国家关系，所以归在政治多极化中的新国关；人类命运共同体是整体愿景，本题得分重心是国与国关系形态。",
        "governance": "材料强调全球治理、共同商量、共同建设、共同分享或规则改革，所以归在政治多极化中的完善全球治理；国际关系民主化侧重各国平等参与，全球治理观侧重治理过程和治理成果。",
        "community": "材料强调命运与共、共同安全、共同发展或共同未来，所以归在政治多极化中的共体；新型国际关系侧重国家关系，命运共同体侧重全人类共同愿景。",
        "unilateral": "材料强调单边主义、贸易保护主义、霸权主义或强权政治，所以归在政治多极化中的单边；民主只写各国平等参与国际事务，本题得分重心是反对单边霸权并坚持真正多边主义。",
        "multi": "材料强调国际力量对比变化、反对阵营对抗或世界格局调整，所以归在政治多极化中的多极化；民主侧重国际事务由各国共同参与，多极化侧重世界格局。",
        "democracy": "材料强调各国平等参与国际事务、反对少数国家垄断事务，所以归在政治多极化中的民主；单边承接反霸权和真正多边主义，本题得分重心是国际关系运行原则。",
        "policy": "材料强调中国特色大国外交、独立自主、和平共处和维护和平促进发展，所以归在中国中的政策；责任强调大国担当，政策强调外交基本立场和宗旨。",
        "wisdom": "材料强调中国理念、中国方案或可复制经验，所以归在中国中的智慧；责任强调行动担当，智慧强调理念方案供给。",
        "responsibility": "材料强调负责任大国、建设性作用、发展中国家身份或公共产品供给，所以归在中国中的责任；智慧强调方案贡献，责任强调行动担当。",
        "history_line": "材料强调综合国力、国际地位、影响力或话语权提升，所以归在中国中的一条历史线；一条主线讲思想指导，本题得分重心是实力和地位变化。",
        "main_line": "材料强调习近平外交思想和新时代中国特色大国外交，所以归在中国中的一条主线；一条历史线讲实力地位，本题得分重心是思想指引。",
        "un_charter": "材料强调联合国宪章、安理会或联合国框架下的行动准则，所以归在联合国原框架术语，落到遵循联合国宪章宗旨原则；联合国平台地位不能替代宪章原则这一规范基础。",
        "un_place": "材料强调联合国平台、多边合作、安理会或联合国体系，所以归在联合国原框架术语，落到践行多边主义的最佳场所；宪章原则强调行为准则，本题得分重心是联合国平台功能。",
    }.get(t.key, f"材料应归在{t.element}中的{t.frame}，围绕“{t.title}”组织答案。")
    return f"材料抓手是：{signal}。{relation}本题得分逻辑是把“{t.title}”与材料事实、设问要求扣合起来。"
...
def run_claudecode(element_files):
    ...
    for element, path in element_files.items():
        log = LOGS / f"claudecode_run_v4_{element}.log"
...
def build_docx(toc_rows):
    ...
    add_text(p, "2026北京高考政治选必一《当代国际政治与经济》\n六要素原框架答题术语宝典（主观题版 V4）", name="黑体", size=20, bold=True, color=(31, 78, 121))
    p = doc.add_paragraph()
    p.alignment = 1
...
def build_mismatch_report(entries):
    rows = []
    focus_sources = ["2025西城期末", "2026朝阳期末", "2025延庆一模", "2024东城二模"]
    for r in entries:
        src = r.get("V3题源", "")
        raw = r.get("题源", "")
        if not any(s in raw or s in src for s in focus_sources):
            continue
        status = "否"
        suggestion = "保留当前位置"
        action = "已核对，未发现需要迁移。"
        if "2025西城期末" in raw and r.get("v3_key") == "unilateral":
            suggestion = "若写企业应对做法，应归经济全球化的开或贸资；本轮仅保留其贸易保护主义识别功能。"
            action = "保留在政治多极化 -> 单边，但答案落点只写欧盟反补贴税属于贸易保护主义，不再写企业出口市场多元化、走出去、投资设厂。"
        elif "2025西城期末" in raw and r.get("V3六要素") == "经济全球化":
            suggestion = "经济全球化 -> 开"
            action = "保留企业应对外部贸易壁垒的开放和规则应对表达。"
        elif "2026朝阳期末" in raw and r.get("v3_key") == "reasonable_concern":
            suggestion = "理论 -> 共同利益 -> 维护本国利益的同时兼顾他国合理关切"
            action = "不新增“国家利益是国际关系决定性因素”术语；只在答案落点中作为维护本国利益的理论补充，并回扣兼顾他国合理关切。"
        elif "2025延庆一模" in raw and r.get("v3_key") == "multi":
            suggestion = "政治多极化 -> 多极化"
            action = "保留在多极化，答案落点只写平等有序的多极化和反对阵营对抗，不再混入兼顾他国合理关切。"
        elif "2024东城二模" in raw and r.get("V3六要素") == "联合国":
            suggestion = "联合国 -> 联合国原框架术语"
            action = "保留联合国条目，答案落点改为联合国平台、维和、安理会协商、国际合作和维护和平安全的卷面句。"
        rows.append({
            "题目标题": src,
            "所在六要素": r.get("V3六要素", ""),
            "所在小框架": r.get("V3小框架", ""),
...
def has_west_xicheng_bad(entries):
    for r in entries:
        if "2025西城期末" in r.get("题源", "") and r.get("v3_key") == "unilateral":
            ans = r.get("V3答案落点", "")
            if any(x in ans for x in ["出口市场多元化", "投资设厂", "开展多种合作"]):
                return True
    return False
...
def write_reports(entries, dup_rows, toc_rows, claude_success, claude_timeout):
    ...
    placeholder_rows = [(pat, final_text.count(pat)) for pat in PLACEHOLDER_PATTERNS]
    template_rows = [(pat, final_text.count(pat)) for pat in TEMPLATE_PATTERNS]
    (AUDIT / "placeholder_language_report.md").write_text(
        "# placeholder_language_report\n\n" + "\n".join(f"- {pat}：{count}" for pat, count in placeholder_rows) + "\n",
        encoding="utf-8",
    )
    (AUDIT / "template_language_report.md").write_text(
        "# template_language_report\n\n" + "\n".join(f"- {pat}：{count}" for pat, count in template_rows) + "\n",
        encoding="utf-8",
    )
    ...
    mismatch_rows = build_mismatch_report(entries)
    write_csv(AUDIT / "question_term_mismatch_report.csv", mismatch_rows, ["题目标题", "所在六要素", "所在小框架", "所在答题术语", "设问", "材料触发点", "答案落点", "是否挂错", "建议归位", "处理方式"])
    mismatch_count = sum(1 for r in mismatch_rows if r["是否挂错"] == "是")
    ...
    (FINAL / "v4_docx_text_extract.txt").write_text(textutil.stdout, encoding="utf-8")
    ...
    concrete_fact_count = final_text.count("具体材料事实")
    preferred_count = final_text.count("本题优先触发")
    scoring_count = final_text.count("踩分词集中在")
    neighbor_count = final_text.count("相邻术语不能直接解释")
    placeholder_sum = sum(count for _, count in placeholder_rows)
    template_sum = sum(count for _, count in template_rows)
    ...
    report = [
        ...
        "4. 正文客观题数量：0",
        f"5. 正文中“具体材料事实”命中数量：{concrete_fact_count}",
        f"6. 正文中“PDF 小框架”命中数量：{pdf_small_count}",
        f"7. 正文中“本题优先触发”命中数量：{preferred_count}",
        f"8. 正文中“踩分词集中在”命中数量：{scoring_count}",
        f"9. 正文中“相邻术语不能直接解释”命中数量：{neighbor_count}",
        f"10. 正文中“考题增补术语 / 考题增补说明”命中数量：{backend_tag_count}",
        f"11. 【答案落点】非卷面语言问题数量：{len(answer_issues)}",
        f"12. 题源格式未统一数量：{unresolved_source + q_format_hits}",
        f"13. 疑似错挂题数量：{mismatch_count}",
        f"14. 非 PDF 术语进入目录、小框架、术语标题、答题术语的数量：{non_surface_count}",
        f"15. 目录是否有页码：{'是' if toc_has_pages else '否'}",
        f"16. 目录是否列入题目：{'是' if toc_question_rows else '否'}",
        f"17. Word 是否成功导出：{'是' if FINAL_DOCX.exists() and zip_ok else '否'}",
        "18. 渲染检查是否完成：QuickLook/textutil fallback 完成；render_docx 因本机缺少 soffice 未完成，详见 render_check.md",
        f"19. ClaudeCode 成功调用次数和超时次数：成功 {claude_success}，超时 {claude_timeout}",
        f"20. 四段式完整率：{complete}/{len(entries)} ({complete/len(entries):.1%})",
    ]
    ...
    if concrete_fact_count:
        fail_reasons.append("正文仍出现具体材料事实")
    ...
    if has_west_xicheng_bad(entries):
        fail_reasons.append("2025西城期末第20题第（2）问单边条目仍写企业出口市场多元化和投资设厂")
    if objective_count:
