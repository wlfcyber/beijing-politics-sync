...
OUTDIR = RECOVERY / "word_render_qa_20260525_batch23_word"
...
def export_pdf_with_word(docx: Path, pdf: Path) -> Path | None:
    backup = None
    if pdf.exists():
        backup = pdf.with_name(f"{pdf.stem}_backup_before_batch23_render_20260525.pdf")
        counter = 1
        while backup.exists():
            backup = pdf.with_name(f"{pdf.stem}_backup_before_batch23_render_20260525_{counter}.pdf")
            counter += 1
...
def reset_outdir() -> None:
    if OUTDIR.exists():
        if OUTDIR.name != "word_render_qa_20260525_batch23_word" or OUTDIR.parent != RECOVERY:
...
def make_contact_sheet(hit_pages: list[int]) -> str | None:
    ...
    out = OUTDIR / "batch23_hit_pages_contact_sheet.png"
    sheet.save(out)
    for thumb in thumbs:
        thumb.close()
    return str(out)
...
def update_global_audit() -> None:
    ...
    for row in rows:
        if row.get("normalized_suite") == SUITE:
            row["blocker_or_next_action"] = "Batch23 matrix/DOCX/render pass; ClaudeCode Opus 4.7 model-lane recheck pending."
...
- Current DOCX / Word-layout visible heading mentions for `2025朝阳期末`: `{manifest['docx_suite_mentions']} / {manifest['word_suite_heading_mentions']}`.
- Raw PDF text extraction exact-string count: `{manifest['pdf_text_suite_mentions']}`.
- Word layout located Batch23 headings on pages `{hit_pages}`.
...
- Hit pages: `{hit_pages}`.
- Manifest: `word_render_qa_20260525_batch23_word/render_manifest.json`.
"""
...
def main() -> int:
    ...
    if len(word_hits) < EXPECTED_HEADINGS:
        raise RuntimeError(f"Expected at least {EXPECTED_HEADINGS} Batch23 headings, found {len(word_hits)}")
    if blank_like_body:
