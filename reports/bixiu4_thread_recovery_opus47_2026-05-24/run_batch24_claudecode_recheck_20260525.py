...
PROMPT = ROOT / "OPUS47_CLAUDECODE_BATCH24_2025_HAIDIAN_MIDTERM_RECHECK_PROMPT.md"
RAW = ROOT / "OPUS47_CLAUDECODE_BATCH24_2025_HAIDIAN_MIDTERM_RECHECK_STREAM_RAW_UTF8.jsonl"
STDERR = ROOT / "OPUS47_CLAUDECODE_BATCH24_2025_HAIDIAN_MIDTERM_RECHECK_STREAM_UTF8_STDERR.log"
DEBUG = ROOT / "OPUS47_CLAUDECODE_BATCH24_2025_HAIDIAN_MIDTERM_RECHECK_STREAM_UTF8_DEBUG.log"
EVIDENCE = ROOT / "OPUS47_CLAUDECODE_BATCH24_2025_HAIDIAN_MIDTERM_RUNTIME_EVIDENCE.json"
RESULT = ROOT / "OPUS47_CLAUDECODE_BATCH24_2025_HAIDIAN_MIDTERM_RECHECK_RESULT.md"
...
BATCH_REPORT = ROOT / "COVERAGE_FUSION_BATCH24_2025_HAIDIAN_MIDTERM_CODEX_20260525.md"
...
def detect_content_result(final_text: str) -> str:
...
def append_status(evidence: dict) -> None:
    ...
    final_status = (
        "LOCAL_CLOSED_NO_BODY_PLACEMENT_PASS_WITH_MODEL_GATE_BLOCKED"
...
"""
    for path in [THREAD_STATUS, GOVERNOR, CONFUCIUS]:
        ...
        text = text.replace(
            "LOCAL_APPLIED_NO_BODY_RENDER_NOT_NEEDED_MODEL_PENDING",
            final_status,
        )
        marker = "\n## Batch24 ClaudeCode Opus 4.7 Recheck: 2025海淀期中"
        if marker in text:
...
"""
    ledger = re.sub(pending_pattern, pending_replacement, ledger, flags=re.DOTALL)
...
"""
    ...
    report = report.replace("Status: `LOCAL_APPLIED_NO_BODY_RENDER_NOT_NEEDED_MODEL_PENDING`", f"Status: `{final_status}`")
    report = report.replace(
        "- ClaudeCode Opus 4.7 recheck is pending for Batch24.",
        f"- ClaudeCode Opus 4.7 recheck completed with `content_result={content}` and `model_gate=BLOCKED_MODEL_CONFIRMATION_REQUIRED`.",
    )
...
def main() -> int:
    ...
    content_result = detect_content_result(final_text)
