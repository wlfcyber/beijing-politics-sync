...
def update_report(manifest: dict[str, object]) -> None:
    ...
    data["target_page_qa"] = {
        "Q2": {
            "page": 34,
            "png": str(RENDER_DIR / "page_034.png"),
            "status": "VISUAL_QA_PASS",
        },
        "Q5": {
            "page": 221,
            "png": str(RENDER_DIR / "page_221.png"),
            "status": "VISUAL_QA_PASS",
        },
    }
...
- Target page inspected: Q2 page `34` (`page_034.png`), Q5 page `221` (`page_221.png`).
...
## Shunyi 2026 Yimo Recovery Render QA 20260525
...
- DOCX/PDF label counts: `{manifest.get('docx_label_count')}/{manifest.get('pdf_label_count')}`.
- Blank-like body pages excluding cover/foreword: `{len(manifest.get('blank_like_pages_excluding_cover_foreword') or [])}`.
- Inserted entries inspected:
  - Q2 page `34`: `word_render_qa_20260525_global_style_norm/page_034.png`.
  - Q5 page `221`: `word_render_qa_20260525_global_style_norm/page_221.png`.
- Evidence boundary: Q2 and Q5 are choice-question official-answer-key/correct-option chains, not main-question scoring-rubric evidence.
...
"""
    ...
    marker = "## Shunyi 2026 Yimo Recovery Render QA 20260525"
    if marker in text:
