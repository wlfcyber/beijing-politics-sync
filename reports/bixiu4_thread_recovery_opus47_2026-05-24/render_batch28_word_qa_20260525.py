...
OUTDIR = RECOVERY / "word_render_qa_20260525_batch28_word"
...
def export_pdf_with_word(docx: Path, pdf: Path) -> Path | None:
    backup = None
    if pdf.exists():
        backup = pdf.with_name(f"{pdf.stem}_backup_before_batch28_render_20260525.pdf")
        counter = 1
        while backup.exists():
            backup = pdf.with_name(f"{pdf.stem}_backup_before_batch28_render_20260525_{counter}.pdf")
            counter += 1
...
def reset_outdir() -> None:
    if OUTDIR.exists():
        if OUTDIR.name != "word_render_qa_20260525_batch28_word" or OUTDIR.parent != RECOVERY:
...
def make_contact_sheet(hit_pages: list[int]) -> str | None:
    ...
    out = OUTDIR / "batch28_hit_pages_contact_sheet.png"
    sheet.save(out)
    for thumb in thumbs:
        thumb.close()
    return str(out)
...
def update_global_audit() -> None:
    ...
    for row in rows:
        if row.get("normalized_suite") == SUITE:
            row["blocker_or_next_action"] = "Batch28 matrix/DOCX/render pass; ClaudeCode Opus 4.7 model-lane recheck pending."
...
- Hit pages: `{hit_pages}`.
- Manifest: `word_render_qa_20260525_batch28_word/render_manifest.json`.
