...
PROMPT = ROOT / "OPUS47_CLAUDECODE_BATCH27_2026_DONGCHENG_FINAL_RECHECK_PROMPT.md"
RAW = ROOT / "OPUS47_CLAUDECODE_BATCH27_2026_DONGCHENG_FINAL_RECHECK_RAW.json"
STDERR = ROOT / "OPUS47_CLAUDECODE_BATCH27_2026_DONGCHENG_FINAL_RECHECK_STDERR.log"
DEBUG = ROOT / "OPUS47_CLAUDECODE_BATCH27_2026_DONGCHENG_FINAL_RECHECK_DEBUG.log"
EVIDENCE = ROOT / "OPUS47_CLAUDECODE_BATCH27_2026_DONGCHENG_FINAL_RUNTIME_EVIDENCE.json"
RESULT = ROOT / "OPUS47_CLAUDECODE_BATCH27_2026_DONGCHENG_FINAL_RECHECK_RESULT.md"
...
BATCH_REPORT = ROOT / "COVERAGE_FUSION_BATCH27_2026_DONGCHENG_FINAL_CODEX_20260525.md"
...
def replace_batch_report(content: str, final_status: str, evidence: dict, content_result: str, local_policy: str) -> str:
    ...
    content = content.replace("Render QA is pending because Batch27 modified the DOCX.", "Render QA passed after Batch27 DOCX modification.")
...
"""
    for path in [THREAD_STATUS, GOVERNOR, CONFUCIUS]:
        ...
        marker = "\n## Batch27 ClaudeCode Opus 4.7 Recheck"
        if marker in text:
...
"""
    ...
    if GLOBAL_AUDIT.exists():
        ...
        text = text.replace(
            "Batch27 matrix/DOCX/render pass; ClaudeCode Opus 4.7 model-lane recheck pending.",
            f"Batch27 matrix/DOCX/render pass; ClaudeCode content_result={content_result}; model gate remains BLOCKED_MODEL_CONFIRMATION_REQUIRED.",
        )
...
def write_blocked_result(started: str, finished: str, return_code: int, stderr_text: str, raw_text: str) -> int:
    evidence = {
        ...
        "result_obj_summary": {"return_code": return_code},
        "model_gate": "BLOCKED_MODEL_CONFIRMATION_REQUIRED",
    }
    EVIDENCE.write_text(json.dumps(evidence, ensure_ascii=False, indent=2), encoding="utf-8", newline="\n")
    RESULT.write_text(
        f"""# ClaudeCode Opus 4.7 Recheck Result - Batch27 2026东城期末
