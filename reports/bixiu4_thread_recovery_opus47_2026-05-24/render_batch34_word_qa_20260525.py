...
r32.SUITE = "2026通州期末"
r32.EXPECTED_HEADINGS = 29
r32.OUTDIR = r32.RECOVERY / "word_render_qa_20260525_batch34_word"
r32.BATCH_REPORT = r32.RECOVERY / "COVERAGE_FUSION_BATCH34_2026_TONGZHOU_FINAL_CODEX_20260525.md"


def reset_outdir() -> None:
    if r32.OUTDIR.exists():
        if r32.OUTDIR.name != "word_render_qa_20260525_batch34_word" or r32.OUTDIR.parent != r32.RECOVERY:
...
def make_contact_sheet(hit_pages: list[int]) -> str | None:
    ...
    out = r32.OUTDIR / "batch34_hit_pages_contact_sheet.png"
    sheet.save(out)
    for thumb in thumbs:
        thumb.close()
    return str(out)
...
def export_pdf_with_word(docx: Path, pdf: Path) -> Path | None:
    backup = None
    if pdf.exists():
        backup = pdf.with_name(f"{pdf.stem}_backup_before_batch34_render_20260525.pdf")
        counter = 1
        while backup.exists():
            backup = pdf.with_name(f"{pdf.stem}_backup_before_batch34_render_20260525_{counter}.pdf")
            counter += 1
        shutil.copy2(pdf, backup)
    word = r32.win32com.client.DispatchEx("Word.Application")
    word.Visible = False
    doc = None
    try:
...
def update_global_audit() -> None:
    ...
    for row in rows:
        if row.get("normalized_suite") == r32.SUITE:
            row["blocker_or_next_action"] = "Batch34 matrix/DOCX/render pass; ClaudeCode Opus 4.7 model-lane recheck pending."
        if row.get("normalized_suite") == "2026石景山期末":
...
- Hit pages: `{hit_pages}`.
- Manifest: `word_render_qa_20260525_batch34_word/render_manifest.json`.
...
r32.export_pdf_with_word = export_pdf_with_word
