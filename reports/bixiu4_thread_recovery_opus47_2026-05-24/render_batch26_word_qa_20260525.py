...
OUTDIR = RECOVERY / "word_render_qa_20260525_batch26_word"
...
def export_pdf_with_word(docx: Path, pdf: Path) -> Path | None:
    backup = None
    if pdf.exists():
        backup = pdf.with_name(f"{pdf.stem}_backup_before_batch26_render_20260525.pdf")
        counter = 1
        while backup.exists():
            backup = pdf.with_name(f"{pdf.stem}_backup_before_batch26_render_20260525_{counter}.pdf")
            counter += 1
...
def reset_outdir() -> None:
    if OUTDIR.exists():
        if OUTDIR.name != "word_render_qa_20260525_batch26_word" or OUTDIR.parent != RECOVERY:
...
def make_contact_sheet(hit_pages: list[int]) -> str | None:
    ...
    out = OUTDIR / "batch26_hit_pages_contact_sheet.png"
    sheet.save(out)
    for thumb in thumbs:
        thumb.close()
    return str(out)
...
def update_global_audit() -> None:
    ...
    for row in rows:
        if row.get("normalized_suite") == SUITE:
            row["blocker_or_next_action"] = "Batch26 matrix/DOCX/render pass; ClaudeCode Opus 4.7 model-lane recheck pending."
...
"""
    ...
    FORMAT_QA.write_text(text + qa_append, encoding="utf-8")
...
- Hit pages: `{hit_pages}`.
- Manifest: `word_render_qa_20260525_batch26_word/render_manifest.json`.
""",
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    manifest = {
        ...
        "word_hit_pages": word_hit_pages,
        "word_heading_hits": word_hits,
    ...
    if rendered_count != pages:
        errors.append(f"rendered_count {rendered_count} != pdf_pages {pages}")
    if body_blank:
        errors.append(f"blank-like body pages detected: {body_blank}")
    if len(word_hits) != EXPECTED_HEADINGS:
        errors.append(f"word heading count {len(word_hits)} != expected {EXPECTED_HEADINGS}")
    if docx_suite_count != EXPECTED_HEADINGS:
        errors.append(f"docx suite count {docx_suite_count} != expected {EXPECTED_HEADINGS}")
    if docx_label_count != pdf_label_count:
        errors.append(f"label mismatch docx/pdf {docx_label_count}/{pdf_label_count}")
    if not hit_pages:
        errors.append("no suite hit pages found by PDF text or Word layout")
    update_global_audit()
    append_reports(manifest)
    print(json.dumps({"manifest": manifest, "errors": errors}, ensure_ascii=False, indent=2))
    if errors:
