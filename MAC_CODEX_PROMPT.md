...
Paste this into Codex on the Mac.
...
1. 先用 wlfcyber 的 GitHub SSH 权限克隆工具包：
   git clone --single-branch --branch california-proxy-mac-kit git@github.com:wlfcyber/beijing-politics-sync.git california-proxy-mac-kit
   cd california-proxy-mac-kit

   如果 clone 失败，请只报告认证/权限问题，不要改用 0cloud，也不要改系统代理。
...
3. 如果 ~/.california-proxy.env 不存在，从 proxy.env.example 复制一份：
   cp proxy.env.example ~/.california-proxy.env
   chmod 600 ~/.california-proxy.env

4. 提醒我填入真实代理配置，或者如果我已经给了你，就帮我写入 ~/.california-proxy.env：
...
9. 如果 Terminal 或 Codex CLI 不走系统代理，执行：
   source ./proxy-env.sh
   curl https://api.ipify.org

10. 请不要把真实 ~/.california-proxy.env、用户名或密码提交到 GitHub。

11. 如果加州代理必须先打开 0cloud 才能访问，请解释这是底层网络无法直连 direct.miyaip.online:8001，不是脚本问题。优先建议使用能独立直连的加州上游，或者让 0cloud 只做底层 TUN/VPN，不要让它把系统代理抢回 7897。
```
