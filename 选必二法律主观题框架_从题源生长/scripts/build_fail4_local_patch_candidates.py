...
CSV_OUT = OUT_DIR / "fail4_local_patch_candidates_20260519.csv"
MD_OUT = OUT_DIR / "fail4_local_patch_candidates_20260519.md"

ROWS = [
    {
        "question_id": "CC0143_2025_朝阳_一模_19",
        "local_patch_decision": "candidate_add_or_revise_core_code",
    ...
    },
    {
        ...
        "local_patch_decision": "boundary_non_core_keep_audit",
    ...
    },
    {
        ...
        "local_patch_decision": "open_container_pending_dual_model",
    ...
    },
    {
        ...
        "local_patch_decision": "exclude_from_xuanbier_core",
...
def write_md():
    ...
    for row in ROWS:
        lines.append(
            f"| `{row['question_id']}` | {row['local_patch_decision']} | {row['proposed_label']} | {row['gate']} | {row['core_use_now']} |"
        )
    ...
    for row in ROWS:
        lines.extend([
            f"### {row['question_id']}",
            "",
            f"- proposed_code_id: `{row['proposed_code_id']}`",
            f"- proposed_label: {row['proposed_label']}",
