请读取已上传的 reasoner_packet_v3_corrected_missing17_20260519.zip。这是正式 v3 修正版输入包，替代此前 35题 trial 包和 v2 full-context 包。

关键口径：
1. 包内共有 70 道 merged canonical 主观题候选。
2. 其中 65 道 formal 可支撑强观察，5 道 reference_only 只能支撑弱观察；missing 已归零。
3. 本轮仍然禁止输出框架；只输出 observation。
4. 每条 observation 必须保留 question_id、rubric_atom_id、material_atom_id；缺编号不得进入代码本。
5. 不要沿用或提及此前 35题 trial 输出，也不要沿用 v2 的 missing17 状态。
6. reference_only 题只能形成弱观察，不能单独支撑核心 code。
