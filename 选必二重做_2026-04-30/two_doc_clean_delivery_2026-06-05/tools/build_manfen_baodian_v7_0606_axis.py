...
V7_PATCH_ENTRY_ID = "E044"
V7_PATCH_TITLE = "2026 · 延庆 · 一模 · 第18题第1问"
PAGE_BREAK_BEFORE_EXAMPLE_ENTRIES = {"E052"}
V7_GLOBAL_TEXT_REPLACEMENTS = [
    (
        "若只笼统写违背诚信原则，得分不如直接写商业诋毁或不正当竞争稳妥。",
        "作答时应明确写出商业诋毁或不正当竞争，不能只停留在诚信原则。",
    ),
    (
        "赡养父母包括经济上供养父母、生活上照料父母、精神上慰藉父母、照顾父母的特殊需要、使父母幸福安度晚年；五点中写出两点即可对应2分。",
        "赡养父母包括经济上供养父母、生活上照料父母、精神上慰藉父母、照顾父母的特殊需要、使父母幸福安度晚年；作答时结合材料写清具体义务。",
    ),
]
V7_PATCH_MATERIAL_APPEND = """名词解释：利用 AI 仿冒公众人物直播营销，是指借助深度合成、语音合成、形象模拟等 AI 技术，在未经授权的情况下，擅自对公众人物的外貌、声音、行为等特征进行高精度模拟，生成与该公众人物高度仿真的虚拟数字人，进而开展直播营销活动，其目的是利用公众人物的社会声誉、粉丝信任关系实现商业流量的不当攫取。
案例：2025 年，北京 M 公司在未经授权的情况下，利用 AI 技术合成著名主持人李某的形象，在粉丝量 88 万的网络账号直播间内，将普通糖果类鱼油包装成具有治疗头晕、改善记忆功效的“神药”开展直播营销，售价 59.9 元/3 瓶，累计售出 17.7 万单，还伪造用户评价。"""


def apply_v7_opus_patch(packets: list[dict]) -> list[dict]:
    image_assets = bt.extract_image_assets()
    for row in packets:
        for old, new in V7_GLOBAL_TEXT_REPLACEMENTS:
            for field in ("material", "prompt", "rubric_clean", "rubric"):
                if row.get(field):
                    row[field] = row[field].replace(old, new)
            if row.get("answer_points"):
                row["answer_points"] = [point.replace(old, new) for point in row["answer_points"]]
        if row.get("entry_id") != V7_PATCH_ENTRY_ID and bt.safe_title(row.get("title", "")) != V7_PATCH_TITLE:
            continue
        row["needs_original_image"] = True
        assets = image_assets.get(bt.canonical_title(row["title"]))
        if assets and assets.get("question"):
            row["image_assets"] = assets
        material = bt.clean(row.get("material", ""))
        if "北京 M 公司" not in material and "北京M公司" not in material:
            row["material"] = f"{material}\n\n{V7_PATCH_MATERIAL_APPEND}"
    return packets
...
def add_question_images(doc: Document, row: dict) -> int:
    ...
    if not paths:
        return 0
    heading = doc.add_paragraph(SOURCE_IMAGE_HEADING, style="Heading 4")
...
def add_example(doc: Document, row: dict) -> tuple[int, int, int]:
    if row.get("entry_id") in PAGE_BREAK_BEFORE_EXAMPLE_ENTRIES:
        doc.add_page_break()
...
def build_docx(packets: list[dict]) -> dict:
    ...
    run.text = "v7 0606 · A轴学生版 · 2024-2026 北京各区主观题"
...
def build_markdown(packets: list[dict]) -> None:
    ...
    lines = [
        ...
        "",
        "v7 0606 · A轴学生版 · 2024-2026 北京各区主观题",
        "",
    ]
...
def write_qa(packets: list[dict], stats: dict, scan: dict) -> None:
    ...
    lines.extend([
        ...
        "- v6 第二轮 Claude Opus 4.8 Max 审核给出系统级 PASS，但要求发布前补齐 2026 延庆一模第18题第1问材料；v7 已补入原题图和关键案情文字。",
    ])
...
def main() -> None:
    packets = sorted(apply_v7_opus_patch(bt.load_packets()), key=bt.sort_key)
